...
class service_equipment(models.Model):
    ...
    base_equipment_id = fields.Many2one('maintenance.equipment', required=True)
    ...
    ement_id = fields.Many2one('service.ement', string='Contract Service', compute="_compute_ement_id")
    ement_type_id = fields.Many2one('service.ement.type', string='ement Type',
                                        related='ement_id.type_id')
    ...
    partner_id = fields.Many2one('res.partner', string='Customer', related='ement_id.partner_id', store=True,
    ...
    group_id = fields.Many2one('service.ement.group', string="Service Group")
    ...
    def _compute_ement_id(self):
        for equipment in self:
            if isinstance(equipment.id, models.NewId):
                return
            ements = self.env['service.ement']
            ement_line = self.env['service.ement.line'].search([('equipment_id', '=', equipment.id)])
            for line in ement_line:
                if line.ement_id.state == 'open':
                    ements = ements | line.ement_id
            if len(ements) > 1:
                msg = _("Equipment %s assigned to many ements.")
                #equipment.message_post(body=msg)

            # daca nu e activ intr-un contract poate se gaseste pe un contract ciorna
            if not ements:
                for line in ement_line:
                    if line.ement_id.state == 'draft':
                        ements = ements | line.ement_id

            if len(ements) > 0:
                equipment.ement_id = ements[0]
                equipment.partner_id = ements[0].partner_id
    ...
    def remove_from_ement_button(self):
        self.ensure_one()
        if self.ement_id.state == 'draft':
            lines = self.env['service.ement.line'].search(
                [('ement_id', '=', self.ement_id.id), ('equipment_id', '=', self.id)])
            lines.unlink()
            if not self.ement_id.ement_line:
                self.ement_id.unlink()
        else:
            raise Warning(_('The ement %s is in state %s') % (self.ement_id.name, self.ement_id.state))
