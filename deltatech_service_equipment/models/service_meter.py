...
class service_meter(models.Model):
    ...
    type = fields.Selection([('counter', 'Counter'), ('collector', 'Collector')], string='Type', default='counter',
                            related="meter_categ_id.type", readonly=True)
    ...
    meter_ids = fields.Many2many('service.meter', 'service_meter_collector_meter', 'meter_collector_id', 'meter_id',
                                 string='Meter', domain=[('type', '=', 'counter')])
    ...
    _sql_constraints = [
        ('equipment_uom_uniq', 'unique(equipment_id,uom_id)',
    ...
    def get_counter_value(self, begin_date, end_date):
        ...
        if self.type == 'counter':
            ...
            res = self.env['service.meter.reading'].read_group(domain, fields=['difference', 'meter_id'],
                                                               groupby=['meter_id'])
            if res:
...
class service_meter_reading(models.Model):
    ...
    meter_id = fields.Many2one('service.meter', string='Meter', required=True, ondelete='cascade',
                               domain=[('type', '=', 'counter')])
